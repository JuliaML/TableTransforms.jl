@testset "OneHot" begin
  ...
  @test typeof(n.a_false) != typeof(categorical(Bool[1, 0, 0, 1, 0, 0]))
  tₒ = revert(T, n, c)
  @test t == tₒ
  ...
  @test typeof(n.b_f) != typeof(categorical(Bool[0, 1, 0, 0, 0, 1]))
  tₒ = revert(T, n, c)
  @test t == tₒ
  ...
  @test typeof(n.c_1) != typeof(categorical(Bool[0, 0, 0, 1, 1, 0]))
  tₒ = revert(T, n, c)
  @test t == tₒ
  ...
  @test n.b_f_ == categorical(Bool[0, 1, 0, 0, 0, 1])
  @test n.b_m_ == categorical(Bool[1, 0, 1, 1, 1, 0])
  tₒ = revert(T, n, c)
  @test t == tₒ
  ...
  @test n.b_f__ == categorical(Bool[0, 1, 0, 0, 0, 1])
  @test n.b_m__ == categorical(Bool[1, 0, 1, 1, 1, 0])
  tₒ = revert(T, n, c)
  @test t == tₒ

  # throws
  a = categorical(categorical(Bool[0, 1, 1, 0, 1, 1]))
  b = ["m", "f", "m", "m", "m", "f"]
  t = Table(; a, b)
